  ...
  const promise = new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      if (w > maxSize || h > maxSize) {
        ...
        resolve({ image: canvas, width: w, height: h });
      } else {
...
class Media {
  ...
  createShader() {
    ...
    loadOptimizedImage(this.image).then(({ image, width, height }) => {
      texture.image = image;
...
class App {
  ...
  onTouchMove(e) {
    ...
    const x = e.touches ? e.touches[0].clientX : e.clientX;
    if (Math.abs(x - this.pointerStartX) > 8) this.didDrag = true;
